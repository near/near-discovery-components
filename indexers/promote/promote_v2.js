...
async function getBlock(block: Block) {
  ...
  async function handlePromotion(
    ...
    if (promotionOperation === "add") {
      ...
      return;
    } else if ( promotionOperation === "remove") {
      console.log("handling remove promotion");
      await _handleRemovePromotion(
        promotion,
        accountId,
        blockHeight,
        blockTimestamp,
    ...
    try {
      ...
      if (posts.length > 0) {
        ...
        const promotionData = {
          ...
          raw_promotion: JSON.stringify(promotion),
  ...
  async function _handleRemovePromotion(promotion,
    accountId,
    blockHeight,
    blockTimestamp,
    receiptId) {
    // complete remove promotion logic
  }
    ...
    await Promise.all(
      nearSocialPosts.map(async (postAction) => {
        ...
        if (
          ...
          // Probably like, unlike, or promote action is happening
          if (
