...
return (
  <RecommendedUsers>
    ...
    {(displayedUsers.length < 1 || displayedUsers == null) &&
      props.scope == "friends" && (
        <p>
          Follow More Users to Unlock More Personalized Recommendations, See
          Who’s{" "}
          <a href="https://near.org/near/widget/PeoplePage?tab=trending">
            Trending
          </a>
        </p>
      )}
    <Profiles>
      {displayedUsers.map((user, index) => (
        <Profile key={index}>
          <Widget
            src={`${REPL_ACCOUNT}/widget/Recommender.Account.AccountProfileCard`}
            props={{
              rank: index,
              accountId:
