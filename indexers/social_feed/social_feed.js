...
async function getBlock(block: Block) {
  ...
  try {
    ...
    const contractActions = actions.filter((action) => action.receiverId === SOCIAL_DB)
    if(!contractActions) {
      console.log('Block has no actions');
      return;
    }
    nearSocialPosts = contractActions.flatMap((action) =>
        action.operations
            ...
            .filter((functionCall) => {
              try {
                if(!functionCall || !functionCall.args || !functionCall.args.data || !functionCall.args.data[0]) {
