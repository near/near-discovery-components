...
async function getBlock(block: Block) {
  ...
  async function handleRepost( accountId, blockHeight, blockTimestamp, receiptId, repostContent) {
    ...
    try {
      ...
      try {
        ...
        const post = posts[0];
        const accountsReposted = post.accounts_reposted.length === 0 ? post.accounts_reposted : JSON.parse(post.accounts_reposted);
    ...
    await Promise.all(
        nearSocialPosts.map(async (postAction) => {
          ...
          if (
            ...
            if (
                Object.keys(postAction.args.data[accountId].index).includes("repost")
            ) {
